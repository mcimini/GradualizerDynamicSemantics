...
sig_typeOfCI = [Decl typeOfCI "o" (Left []) [Simple "term", Simple "term", Simple "typ"]]

encodingSuffix :: String
encodingSuffix = "'"

addCastInsertion :: TypeSystem -> TypeSystem
addCastInsertion (Ts sig rules) = (Ts (sig ++ sig_typeOfCI) (nub (rules ++ (map (castInsertion_rule sig) (filter (filterRules typeOfGradual) rules)))))
...
castInsertion_prem (Formula pred strings interms outterms) = if pred == typeOfGradual then (Formula typeOfCI strings interms ((enc pred (head interms)):outterms)) else (Formula pred strings interms outterms)
castInsertion_prem (Hypothetical premise1 premise2) = (Hypothetical (castInsertion_prem premise1) (castInsertion_prem premise2))
castInsertion_prem (Generic premise) = (Generic (castInsertion_prem premise))
...
encWithCasts pkg pred (Bound variable) = (Bound variable)

castWrap :: Package -> String -> Term -> Term -> Term -> Term 
castWrap pkg pred (Var variable) source target = 
	if (isAbstraction pkg (Var variable)) 
		then (Lambda "x" (cast (Application (Var variable) (appliedTermTo (pkg_getRule pkg) (Var variable)) ))) 
		else cast (Var variable) 
		where 
			cast = \x -> makeCastTerm (enc pred x) source target
...
appliedTermTo rule term = case searchPremiseByPredAndVar rule typeOf term of 
	Nothing -> error "impossible case because assignedTypeByVar succeded" 
	Just (Formula pred strings interms outterms) -> case head interms of (Application t1 t2) -> t2 

subtypeUnfold :: Premise -> [Premise]
subtypeUnfold (Formula pred strings interms outterms) = if pred == "subtypeG" then [Formula "subtypeCI" [] interms [freshvar], Formula consistency [] [interms !! 0, freshvar] []] else [(Formula pred strings interms outterms)] where freshvar = Var (head strings) 
