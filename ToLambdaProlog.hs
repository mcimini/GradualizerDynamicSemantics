...
toLambdaPrologSig :: Signature -> String
toLambdaPrologSig sig = "kind label\t type.\n\n" ++ (concat (map toLambdaPrologSig_entry sig))

toLambdaPrologSig_entry :: SignatureEntry -> String
toLambdaPrologSig_entry (Decl c typ info entries) = "type " ++ c ++ "\t\t" ++ (intercalate " -> " ((map toLambdaPrologSig_TE entries) ++ [typ])) ++ ".\n"
...
toLambdaPrologPr (Formula pred info interms outterms) = if pred == "=" then (intercalate " is " (map toLambdaPrologTerm interms)) else (toLambdaPrologPredicateName pred info) ++ displayInputs ++ displayOutputs
                 where
...
toLambdaPrologPr (Generic premise) = "(pi x\\ " ++ (toLambdaPrologPr premise) ++ ")"
